...
export function FirmwareTab() {
  const {
    send, firmwareVersion, firmwareBuildDate,
    dfuProgress, dfuError, clearDFU,
    flashFirmwareFromSerial, flashFirmwareDirect,
  } = useMotorStore()
    ...
    <div className="space-y-4 max-w-lg">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
        ...
        {!isDone && !isFlashing && (
          <>
            <p className="text-xs text-slate-400 mb-3">
              Sends a DFU command to the device, waits for it to reboot into the STM32 bootloader,
              then flashes via WebUSB — no tools needed.
            </p>
            ...
            {(dfuError) && (
              <div className="flex gap-2 p-3 bg-red-500/10 border border-red-500/30 rounded-lg mb-3 text-sm text-red-300">
                ...
                <div>
                  ...
                  <div className="font-mono text-xs mt-1 break-all">{dfuError}</div>
                </div>
              </div>
            )}

            <div className="flex gap-2">
              {/* Primary: send DFU via serial then flash */}
              <button
                onClick={() => firmwareFile && flashFirmwareFromSerial(firmwareFile)}
              ...
              {/* Secondary: device already in DFU */}
              <button
                onClick={() => firmwareFile && flashFirmwareDirect(firmwareFile)}
                disabled={!firmwareFile || !webUsbSupported}
                ...
                <span className="text-xs whitespace-nowrap">Already in DFU</span>
              </button>
            </div>
          </>
        )}
        ...
        {isDone && (
          <div className="space-y-3">
            ...
            <button onClick={() => { clearDFU(); setFirmwareFile(null) }}
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
        ...
        <ol className="space-y-1.5 text-xs text-slate-500">
          {['Unplug motor from power and USB', 'Hold DFU button (small button on back)', 'Plug in USB-C while holding', 'Release after 2 seconds', 'Use "Recover Device" on the connect screen or dfu-util'].map((s, i) => (
